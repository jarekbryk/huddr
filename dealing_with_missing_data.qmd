...
In R, missing values are represented by a dedicated symbol `NA` (with variants like `NA_integer_`, `NA_real_` and `NA_character_`). It also has separate concepts for “not a number” (`NaN`) and “infinite” (`Inf`) values.

From a user point of view, you need to know how to make sure that missing data is properly recognised by R, and how to accommodate it when operating on data with missing values. The latter is particularly visible due to the fact that in R, missing values "contaminate" objects downstream - any operation on a missing value (or a set that includes even a single missing value), results in `NA`. For example:
...
```{r}
# I am creating a tiny dataframe to illustrate the point about removing missing values
df <- data.frame(
...
(You may want to check [`base R`](tidyverse_vs_baser.qmd) functions such as `complete.cases()` and `na.omit()` as well.)
...
If your data is large, has a complicate structure of missingness, or you want to quickly visualise the pattern of missing data, you should check out [a package `naniar`](https://naniar.njtierney.com/articles/naniar.html) by Nicholas Tierney. Not only it contains convenient functions to normalise missing data in an object (e.g. `replace_with_na()`), or to have a summary of variables with missing values (e.g. `prop_miss_var()`), but the package website contains useful guide of how to look at missingness in data.
...
**Functions covered:** `is.na()`, `!is.na()`, `drop_na()`, `filter()`, `mean()` / `sd()` with `na.rm = TRUE`

**Key idea:** `NA` is contagious — one missing value makes the whole result `NA`. You have to actively decide what to do with missing data (ignore it with `na.rm = TRUE`, remove rows with `drop_na()`, or filter with `!is.na()`) before your summaries will work.
:::
>>>>>>> d92c1ff66332d7dbcf16fc95c7a54db236ed78bb
